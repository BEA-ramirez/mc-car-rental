...
import { MoreHorizontal, Eye, Car, Loader2 } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function PartnerUnits({
  ...
  return (
    <div className="flex flex-col h-full w-full bg-transparent relative transition-colors duration-300">
      {/* Header Info (Kept just the total count aligned right) */}
      ...
      <div className="flex-1 min-h-[300px] w-full relative">
        {isLoading ? (
          ...
          <div className="absolute inset-0 overflow-y-auto pr-2 custom-scrollbar transition-colors">
                ...
                >
                  ...
                  <DropdownMenu modal={false}>
                    ...
                    >
                      <DropdownMenuItem
                        onClick={() =>
                          router.push(`/admin/units/${car.car_id}`)
                        }
                        className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground focus:bg-secondary focus:text-foreground cursor-pointer rounded-lg transition-colors py-2"
                      >
